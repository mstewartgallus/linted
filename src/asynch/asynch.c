...
int linted_asynch_pool_create(struct linted_asynch_pool * pool)
{
    ...
    struct linted_queue * command_queue;
    if ((errnum = linted_queue_create(&command_queue,
                                      sizeof (union linted_asynch_task))) != 0) {
        return errnum;
    }

    struct linted_queue * event_queue;
    if ((errnum = linted_queue_create(&event_queue,
                                      sizeof (union linted_asynch_event))) != 0) {
        goto destroy_command_queue;
    }

    struct linted_asynch_worker_pool * worker_pool = malloc(sizeof *worker_pool +
                                                            POOL_SIZE * sizeof worker_pool->workers[0]);
    if (NULL == worker_pool) {
        errnum = errno;
...
destroy_event_queue:
    linted_queue_destroy(event_queue);

destroy_command_queue:
    linted_queue_destroy(command_queue);
                                       ...
                                       union linted_asynch_task * task)
{
    ...
    // sized thread pool.
    do {
        errnum = linted_queue_send(pool->command_queue, task);
    } while (EINTR == errnum);
    return errnum;
}
                                     ...
                                     size_t * event_count)
{
    ...
    if ((errnum = linted_queue_recv(pool->event_queue, &events[0])) != 0) {
        return errnum;
    }
...
static void * worker_routine(void *arg)
{
    ...
    for (;;) {
        ...
        do {
            recv_errnum = linted_queue_recv(worker_pool->command_queue,
                                            &task);
        ...
        case LINTED_ASYNCH_TASK_READ: {
            ...
            do {
                ...
                for (;;) {
                    ssize_t result = read(task_read->ko, task_read->buf + bytes_read,
                                          bytes_left);
                    if (-1 == result) {
                        errnum = errno;
                        break;
        ...
        case LINTED_ASYNCH_TASK_MQ_RECEIVE: {
            struct linted_asynch_task_mq_receive * task_receive = &task.mq_receive;
            size_t bytes_read = 0;
            linted_error errnum;
            do {
                {
                ...
                ssize_t result = mq_receive(task_receive->ko,
                                            task_receive->buf,
                                            task_receive->size,
                                            NULL);
                if (-1 == result) {
        ...
        do {
            send_errnum = linted_queue_send(worker_pool->event_queue,
                                            &event);
