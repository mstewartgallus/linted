...
int main(int argc, char **argv) {
    ...
    if (-1 == child_pid) {
        ...
        if (-1 == request_close_status) {
            LINTED_ERROR("Could not close fork server request reader end: %s\n",
                         strerror(errno));
        }
...
static int fork_server_run(linted_task_spawner_t const spawner,
    ...
    for (;;) {
        ...
        if (-1 == bytes_read) {
            LINTED_ERROR("Could not read bytes from fork request socket: %s\n",
                         strerror(errno));
        }
...

        {
            ...
            if (-1 == reply_write_status) {
                LINTED_ERROR("Fork server could not write reply to child requester: %s\n",
                             strerror(errno));
            }
        }
        ...
        int const reply_close_status = close(reply_writer);
        if (-1 == reply_close_status) {
            LINTED_ERROR("Fork server could not close reply file descriptor: %s\n",
                         strerror(errno));
        }
    }
    ...
    if (-1 == request_close_status) {
        LINTED_ERROR("Could not close read end of fork request socket: %s\n",
                     strerror(errno));
    }
