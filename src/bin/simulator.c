...
int linted_simulator_send_tick(struct linted_simulator_tick_results * const tick_results,
...

    {
        ...
        do {
            bytes_written = sendmsg(simulator._inbox,
                                    &message,
                                    0);
    ...
    for (;;) {
        ...
        case SIMULATOR_TICK: {
            ...
            struct reply_data const reply_data = {
                .tick_results = (struct linted_simulator_tick_results) {
            ...
            if (-1 == bytes_written) {
                LINTED_ERROR("Could not read from simulator inbox: %s\n",
                             strerror(errno));
            }
            break;
        }
