...
package body Linted.Mod_Atomics is
   Last : constant Element_T := Element_T'Last;
   ...
   protected body Atomic is
      ...
      procedure Saturating_Increment is
      begin
	 if Ptr < Last then
	    Ptr := Ptr + 1;
	 end if;
      ...
      procedure Saturating_Decrement is
      begin
	 if Ptr > 0 then
	    Ptr := Ptr - 1;
	 end if;
      end Saturating_Decrement;
   end Atomic;
   ...
   procedure Saturating_Increment (A : in out Atomic) is
   begin
      A.Saturating_Increment;
   end Saturating_Increment;

   procedure Saturating_Decrement (A : in out Atomic) is
   begin
      A.Saturating_Decrement;
   end Saturating_Decrement;
